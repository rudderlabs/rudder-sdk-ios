...
class RudderCore {
    ...
    init(
        ...
        self.flushPolicies = [
            CountBasedFlushPolicy(flushQueueSize: configuration.flushQueueSize)
        ]
...
extension RudderCore {
    ...
    func reset(and refreshAnonymousId: Bool) {
        if refreshAnonymousId {
            let anonymousId: String = .getUniqueId()
            userDefaultsWorker.write(.anonymousId, value: anonymousId)
        }
        reset()
    }
...
extension Message {
    internal func applyRawEventData(userInfo: UserInfo?) -> Self {
        ...
        result.messageId = .getUniqueId()
        result.timestamp = .getTimestampString()
